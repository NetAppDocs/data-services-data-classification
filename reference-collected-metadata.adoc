...
By default, if Data Classification doesn't have these permissions, the system won't scan those files in your volumes because Data Classification can't revert the last access time to the original timestamp. If your data can tolerate the last access time not being reset to the original timestamp, enable the *Scan without write permissions* in the Configuration settings. 
...
* "Classified and updated last access time" - This shows the files that were classified and Data Classification was unable to reset the last access time back to the original date. This filter is relevant only for environments where *Scan without write permissions* is enabled. 

If needed, you can export these results to a report so you can see which files are, or aren't, being scanned because of permissions. link:task-investigate-data.html#download-your-report[Learn more about Data Investigation reports^].
