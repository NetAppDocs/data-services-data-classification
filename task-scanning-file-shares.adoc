...
. Decide how you want to scan the volumes. link:concept-classification.html#whats-the-difference-between-mapping-and-classification-scans[Learn about mapping and classification scans].
...
[NOTE]
The system overview pages has a for *Scan without without permissions* option; it's disabled by default. This means that if Data Classification doesn't have write attributes permissions in CIFS or write permissions in NFS, the system won't scan the files because Data Classification can't revert the "last access time" to the original timestamp. +
If you check the *Scan without without permissions* option*, the scan resets the last accessed time and scans all files regardless of permissions. +
