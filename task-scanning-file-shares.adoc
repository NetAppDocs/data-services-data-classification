...
[NOTE]
The system overview page has a for *Scan without write permissions* option that is disabled by default. This means that if Data Classification doesn't have write attributes permissions in CIFS or write permissions in NFS, the system won't scan the files because Data Classification can't revert the "last access time" to the original timestamp. +
If you check the *Scan without write permissions* option, the scan resets the last accessed time and scans all files regardless of permissions. +
