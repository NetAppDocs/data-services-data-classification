...
* *Map-only scans* provide only a high-level overview of your data. Mapping can be done on your data sources very quickly because it does not access files to see the data inside.
* *Map and classify* scans provide deep-level scanning of your data. 

You might benefit from using Map-only scans to quickly scan your data to identify the data sources that require more research and then  later enable Map and classify scans only on those required data sources or volumes.

The priority is assigned only to Map-only scans, not Map and classify scans. 
