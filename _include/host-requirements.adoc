* Data Classification must be on a dedicated host. The host cannot be shared with other applications or third-party software such as antivirus. 
...
* When deploying a compute instance in the cloud for your Data Classification installation, use a system that meets the "Large" system requirements above:
...
** *Azure VM size*: "Standard_D16_v5". link:reference-instance-types.html#azure-instance-types[See additional Azure instance types^].
...
*** Red Hat Enterprise Linux version 8.8, 8.10, 9.0, 9.1, 9.2, 9.3, 9.4, 9.5, 9.6, and 9.7.

** Advanced Vector Extensions (AVX2) must be enabled on the host system.

* *Red Hat Subscription Management*: The host must be registered with Red Hat Subscription Management. If it is not registered, the system cannot access repositories to update required third-party software during installation.
