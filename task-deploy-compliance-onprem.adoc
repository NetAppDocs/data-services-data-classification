...
summary: To install NetApp Data Classification on a Linux host in your network or on a Linux host in the cloud that has internet access, you need to deploy the Linux host manually in your network or in the cloud.
---
= Install NetApp Data Classification on a host with internet access
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
To deploy NetApp Data Classification on a Linux host in your network or on a Linux host in the cloud that has internet access, you need to deploy the Linux host manually in your network or in the cloud.
...
If you don't already have a Console agent , https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-on-prem.html[deploy the Console agent on-premises^] on a Linux host in your network, or on a Linux host in the cloud.
...
A Console agent is required before you can install and use Data Classification. In most cases you'll probably have a Console agent set up before you attempt to activate Data Classification.
...
//As you can see, there may be some situations where you need to use https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html[multiple Console agents].
...
| \https://api.console.netapp.com | Communication with the Console, which includes NetApp accounts.
...
In most situations, you install Data Classification on a single-host system. This is not the same VM host as the Console agent. <<Single-host installation for typical configurations,See those steps here>>.
...
. Download the Data Classification software from the https://mysupport.netapp.com/site/products/all/details/cloud-data-sense/downloads-tab/[NetApp Support Site^]. Select the file named *DATASENSE-INSTALLER-<version>.tar.gz*.
