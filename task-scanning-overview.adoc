...
After the initial scan, Data Classification continuously scans your data in a round-robin fashion to detect incremental changes. Scans are automatically conducted every day. Data Classification conducts scans in batches of six; you can pause scans or link:task-managing-repo-scanning.html#prioritize-scans[manually prioritize them].
...
[TIP]
Performing a map-only scan can be helpful in gaining an overview of your data. With this high-level overview, you can then decide which data sources you want to conduct full scans on. 
...
| Ability to see metadata from files** | Yes | Yes
|===

{asterisk} Data Classification does not impose a limit on the amount of data it scans. Each Console agent supports scanning and displaying 500 TiB of data. To scan more than 500 TiB of data, link:https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html#connector-installation[install another Console agent^] then link:task-deploy-overview.html[deploy another Data Classification instance]. +
