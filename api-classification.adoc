...
[.lead]
Use the NetApp Data Classification REST API to access the same capabilities that are available in the web UI.
...
** Data is exported in JSON format (easy to parse and push to third-party applications, such as Splunk)

* Create queries using "AND" and "OR" statements, include and exclude information, and more.
+
For example, you can locate files _without_ specific personally identifiable information (PII) (functionality not available in the UI). You can also exclude specific fields for the export operation.
...
To access Swagger, you need the IP address of your Data Classification instance. For a cloud deployment, use the public IP address. Then you need to access this endpoint: 
...
You need to get all the relevant fields and options for a system to view all of the filters in the Investigation tab.
...
Use the response in the request parameters to filter the files you want to copy. 
...
This API lets you create multiple actions.
