...
Data Classification supports two types of custom classifiers: categories and personal identifiers. Custom categories are created based on a set of files that you upload from which Data Classification creates an AI model to identify similar data in your organization (for example, a health research firm might create a clinical analysis category). Custom personal identifers are created with keyword lists or a regular expression (regex) to identify information specific to your organization that can pose a compliance risk. 
...
. Select *Personal identifier* then **Next**.
+
...
After you create a custom personal identifer, it's results are captured in the next scheduled scan. To capture results sooner, perform an on-demand scan.  To view results, see xref:task-generating-compliance-reports.html[Generate compliance reports].
...
// mask results -- detected personal results will be masked
...
. Define the logic for your custom category by with a collection of text-based files that satifies the requirements. Provide the IP address of the *Working address* then select the *Volume* from the dropdown menu. 
+
Enter the **Directory path** for the directroy that contains the training data. 
...
.Result 
After you create a custom category, it's results are captured in the next scheduled scan. To capture results sooner, perform an on-demand scan. 
...
.Steps
. In NetApp Data Classification, select *Custom classification*. 
. Identify the classifier you want to delete the select the action menu `...` at the end of it row. 
. Select **Edit logic**. 
