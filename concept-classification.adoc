...
. You enable high-level mapping (called _Mapping only_ scans) or deep-level scanning (called _Full_ scans) on one or more data sources.
...
- **Map-only scans** provide only a high-level overview of your data and are performed on selected data sources. Map-only scans take less time than full scans because the do not access files to see the data inside. You might want to do this initially to identify areas of research and then perform a full scan on those areas.
