...
---
= Integrate Active Directory with NetApp Data Classification
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
* You can see link:task-investigate-data.html[full file permissions] for each file and directory when you select the "View all Permissions" button.
...
For CIFS volumes/file shares, if you want to make sure your files "last accessed times" are unchanged by Data Classification scans, the user should have Write Attributes permission. If possible, we recommend making the Active Directory configured user part of a parent group in the organization which has permissions to all files.
...
. In the Active Directory heading, select **Add**. 
. From the Data Classification Configuration page, select *Add Active Directory*.

. In the Connect to Active Directory dialog, enter the Active Directory details then select *Connect*.
+
...
If you need to modify any values in your Active Directory integration, you must update the Active Directory configuration in Data Classification. Return to the **Configuration** menu in Data Classification then switch to the settings tab to make the changes. 

You can also delete the integration selecting the **Remove** button under Active Directory. 