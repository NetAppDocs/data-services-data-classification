...
TIP: There is no option to select map-only scans for database schemas.

. From the Configuration page, select the *Manage* button for the database you want to configure.
+
...
. For each schema you want to enable scans for, select the *Scan type* column then set the scan to off or **Full scan**. 
