...
---
= Install NetApp Data Classification on a Linux host in private mode
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use _private mode_ to install NetApp Data Classification on a Linux host in an on-premises site without internet access (also known as an offline installation or a dark site). This deployment uses an installation script and keeps the Data Classification instance disconnected from the NetApp Console SaaS layer, allowing you to scan local data sources in an isolated environment.

link:https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html[Learn about the different deployment modes for the Console agent and Data Classification^].

include::_include/ova-deployment.adoc[]

The Data Classification installation script checks whether the system and environment meet the required prerequisites. If they do, installation starts. If you would like to verify the prerequisites independently of running the Data Classification installation, you can download a separate software package that tests only the prerequisites. link:task-test-linux-system.html[See how to check if your Linux host is ready to install Data Classification].

== Supported data sources

When installed in private mode, Data Classification can only scan data from data sources that are also local to the on-premises site. At this time, Data Classification can scan the following *local* data sources:
...
Migrating the Console agent to a different host isn't supported when Data Classification is deployed on a Console agent. If you need Classification on a different Console agent in a dark site, deploy a new Console agent and Data Classification instance on the new host. 
...
If you don't already have a Console agent installed in private mode, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-private-mode.html[deploy the Console agent^] on a Linux host now.
...
If you don't already have a Console agent installed in private mode, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-private-mode.html[deploy the Console agent^] on a Linux host in your offline site.
...
[NOTE]
The Data Classification installer reserves UID 4321 and GID 4321. Before you start the installation, ensure that UID 4321 and GID 4321 aren't in use on the host machine. If they are in use, the installation will fail with an "invalid user" error message.
...
* Ensure that the Console agent has permissions to deploy resources and create security groups for the Data Classification instance. You can find the latest Console permissions in https://docs.netapp.com/us-en/console-setup-admin/reference-permissions.html[the policies provided by NetApp^].
...
In most situations, you install Data Classification on a single-host system. This is not the same VM host as the Console agent. 
...
* We recommend that your Console agent software is upgraded to the newest available version. https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html[See the Console agent upgrade steps^].
