...
[.lead]
Use _private mode_ to install NetApp Data Classification on a Linux host in an on-premises site without internet access. This deployment uses an installation script and keeps the Data Classification instance disconnected from the NetApp Console SaaS layer, allowing you to scan local data sources in an isolated environment.
