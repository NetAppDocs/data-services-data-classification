...
Data Classification is part of the NetApp Console core features. You don't need to use the marketplaces to use Data Classification. 
...
Scans are persistent. If you restart a VM while there are active scans, the scan will continue from the point where it left after the VM reboots successfully. 
...
The map-only scan doesn't heat up the cold data: it stays cold and remains in object storage. On the other hand, if you perform the full scan, some configurations might heat up the cold data. 
