...
---
= FAQ for NetApp Data Classification
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use this FAQ to find quick answers to common questions about NetApp Data Classification.

== Data Classification basics

.How does Data Classification work?

Data Classification deploys another layer of AI alongside your NetApp Console system and storage systems. It then scans the data on volumes, buckets, databases, and other storage accounts and indexes the data insights it finds. Data Classification leverages both artificial intelligence and natural language processing, as opposed to alternative solutions that are commonly built around regular expressions and pattern matching. 
...
Data Classification is part of the NetApp Console core features. You do not need to deploy Data Classification from a cloud marketplace.
...
When scanning data, the NetApp Console agent needs to be installed in the following locations: 
...
If you have data in these locations, you may need to use https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html#when-to-use-multiple-connectors[multiple Console agents^].
...
* In AWS, Data Classification runs on an m6i.4xlarge instance with a 500 GiB GP2 disk. 
...
.Can I migrate the Console agent or Data Classification instance to a different host in a dark site deployment? 

No. Migrating the Console agent to a different host isn't supported when Data Classification is deployed on a Console agent. If you need Classification on a different Console agent in a dark site, deploy a new Console agent and Data Classification instance on the new host. 

== Data Classification management

The following questions provide information on how to manage Data Classification.
...
When Data Classification scans volumes that have cold data tiered to object storage using the map-only scans, it scans all of the data--data that's on local disks and cold data tiered to object storage. This is also true for non-NetApp products that implement tiering.

The map-only scan doesn't heat up the cold data: it stays cold and remains in object storage. On the other hand, if you perform the full scan, some configurations can heat up the cold data. 
...
When Data Classification detects personally identifiable information (PII), or when it performs a DSAR search, only the following file formats are supported:

`+.CSV, .DCM, .DOC, .DOCX, .JSON, .PDF, .PPTX, .RTF, .TXT, .XLS, .XLSX, Docs, Sheets, and Slides+`

.What kinds of data and metadata does Data Classification capture?

Data Classification enables you to run a map-only scan or a full scan on your data sources. Map-only scans provide only a high-level overview of your data, whereas full scans provide deep-level scanning of your data. Map-only scans can be done on your data sources very quickly because they do not access files to see the data inside.

* *Map-only scan*: Data Classification scans the metadata only. This is useful for overall data management and governance, quick project scoping, very large estates, and prioritization. Data mapping is based on metadata and is considered a *fast* scan.
+
...
* *Full scan*: Data Classification scans data using standard protocols and read-only permission throughout your environments. Select files are opened and scanned for sensitive business-related data, private information, and issues related to ransomware.
+
...
Data Classification can identify many types of private data such as personal information (PII) and sensitive personal information (SPII). For details about private data, refer to xref:reference-private-data-categories.html[Categories of private data that Data Classification scans].
...
Scan performance can vary based on the network bandwidth and the average file size in your environment. It can also depend on the size of the size and location of host system (that is, whether it's in the cloud or on-premises). See link:concept-classification.html[The Data Classification instance] and link:task-deploy-overview.html[Deploying Data Classification] for more information.

When initially adding new data sources, you can also choose to perform only a map-only scan or a full scan. Mapping can be done on your data sources very quickly because it does not access files to see the data inside. link:task-scanning-overview.html[Understand the difference between a mapping and classification scan].
...
Scans persist. If you restart a VM while scans are running, they resume from where they left off after the VM reboots successfully.

== Privacy and data access 

The following questions relate to Data Classification's handling of private data and data access. 
...
No. The private data sent between your browser and the Data Classification instance is secured with end-to-end encryption using TLS 1.2, which means NetApp and third parties cannot read it. Data Classification does not share any data or results with NetApp unless you request and approve access.
